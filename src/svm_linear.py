...
DATA_DIR = ROOT / "data"
MODEL_PATH = DATA_DIR / "best_svm_linear.pkl"
RESULTS_PATH = DATA_DIR / "svm_linear_results.json"
...
for combo, C in enumerate(progress_bar(C_values, desc="Linear SVM grid"), start=1):
    print(f"\n[{combo}/{len(C_values)}] Training linear SVM - C={C}...")
    svm = SVC(kernel="linear", C=C, decision_function_shape="ovr")
    svm.fit(X_train, y_train)
    print(f"[{combo}/{len(C_values)}] Evaluating on val set...")
    val_top1, val_top5 = evaluate(svm, X_val, y_val, "Val")

    marker = " <-- best so far" if val_top1 > best_val_top1 else ""
    print(
        f"[{combo}/{len(C_values)}] C={C} - "
...
print(f"Saved linear SVM metrics to {RESULTS_PATH}")
